...
public class World {
    ...
    public void load_tiles() {
        ...
        try {
            ...
            f.close();
            int x_max = rows.get(0).split("\t").length - 1;
        ...
        if ((x >= 0 && x < _world.length) && (y >= 0 && y < _world[0].length) && _world[x][y] != null) {
            switch (_world[x][y]) {
                case "StartingRoom":
                    ...
                    break;
                case "OxygenRoom":
                    mt = new OxygenRoom(x, y, new Oxygen(250));
                    mt = checkRoomExists(mt);
                    break;
                case "ThunderRoom":
                    mt = new ThunderRoom(x, y, new Thunderman("Thunderman",
                            "Attacks the player by turning either to a tornado or thunderstorm",
                            580, 411));
                    mt = checkRoomExists(mt);
                    break;
                case "BlackHolePath":
                    mt = new BlackHolePath(x, y);
                    mt = checkRoomExists(mt);
                    break;
                case "LeaveThunderRoom":
                    mt = new LeaveThunderRoom(x, y);
                    mt = checkRoomExists(mt);
                    break;
            }
        }
